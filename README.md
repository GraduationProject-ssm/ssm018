...
### 点击播放视频 ▼
[![Watch the video](https://i.sstatic.net/Vp2cE.png)](https://www.bilibili.com/video/BV1T48XecE9G?p=17)
...
# 4 系统设计
营业厅宽带系统的设计方案比如功能框架的设计，比如数据库的设计的好坏也就决定了该系统在开发层面是否高效，以及在系统维护层面是否容易维护和升级，因为在系统实现阶段是需要考虑用户的所有需求，要是在设计阶段没有经过全方位考虑，那么系统实现的部分也就无从下手，所以系统设计部分也是至关重要的一个环节，只有根据用户需求进行细致全面的考虑，才有希望开发出功能健全稳定的程序软件。
## 4.1 系统概要设计
...
| :-: | :-: | :-: | :-: |
|id|int(11)|id|是|
|kdname|varchar(255)|宽带名称|否|
|notice\_content|varchar(255)|套餐描述|否|
|money|decimal(10,0)|宽带价格|否|
|money\_time|timestamp|缴费时间|否|
|speed|int(11)|宽带速度|否|
...
| :-: | :-: | :-: | :-: |
|id|int(11)|id|是|
|yh\_types|tinyint(255)|所属用户|否|
|family|varchar(255)|安装地址|否|
|number|varchar(255)|联系电话|否|
|kd\_types|tinyint(255)|套餐信息|否|
|create\_time|timestamp|安装时间|否|
|mariadb\_types|tinyint(255)|是否安装|否|
...
| :-: | :-: | :-: | :-: |
|id|int(11)|id|是|
|note|varchar(255)|留言信息|否|
|yhnote|varchar(11)|留言人|否|
|note\_time|timestamp|留言时间|否|
|reply|varchar(255)|回复|否|
|glreply|varchar(11)|回复人|否|
|reply\_time|timestamp|回复时间|否|
...
| :-: | :-: | :-: | :-: |
|id|int(11)|id|是|
|name|varchar(255)|用户名称|否|
|account|varchar(255)|账号|否|
|password|varchar(255)|密码|否|
|img\_photo|varchar(255)|头像|否|
|balance|decimal(10,2)|余额|否|
|role|varchar(255)|身份|否|
|number|varchar(11)|手机号|否|
|family|varchar(255)|住址|否|
|identity|varchar(255)|身份证号|否|
...
|biaoti|varchar(200)|标题|否|
|leixing|varchar(200)|类型|否|
|neirong|longtext|内容|否|
|riqi|datetime|日期|否|
# 5 系统实现
...
图5.1 账号管理页面
### 5.2 用户信息管理
如图5.2显示的就是用户信息管理页面，此页面提供给管理员的功能有：查看已发布的用户信息，修改用户信息或更换用户内容，用户信息作废，即可删除。
...
图5.2 健身房信息管理页面
### 5.3 宽带类型管理
如图5.3显示的就是宽带类型管理页面，此页面提供给管理员的功能有：对宽带类型的新增、修改、删除功能
...
图5.3 宽带类型管理页面
### 5.4 宽带信息列表
如图5.4显示的就是宽带信息列表页面，此页面提供给管理员的功能有：对全部的宽带信息的一个管理、可以根据宽带信息名称进行模糊查询。
